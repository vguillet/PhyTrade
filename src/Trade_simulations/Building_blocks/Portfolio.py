...
# Own modules
from src.Trade_simulations.Trading_bots.Tradebot_multi_ticker import Tradebot
...
class Portfolio:
    ...
    def perform_trade_run(self,
        ...
        for i in range(self.data_slice_length):
            ...
            for ticker in self.content.keys():
                if self.content[ticker]["Individual"].analysis.trade_signal[i] == 1:
                    sell_orders.append(ticker)

                elif self.content[ticker]["Individual"].analysis.trade_signal[i] == 0:
                    hold_orders.append(ticker)

                elif self.content[ticker]["Individual"].analysis.trade_signal[i] == -1:
            ...
            for orders in order_lst:
                for k in range(len(orders)):
                    for j in range(1, len(orders)):
                        if abs(self.content[orders[j]]["Individual"].analysis.trade_spline[i]) > \
                                abs(self.content[orders[j-1]]["Individual"].analysis.trade_spline[i]):
            ...
            for orders in order_lst:
                ...
                for ticker in orders:
                    self.tradebot.perform_trade(ticker=ticker,
                                                ...
                                                signal_strength=self.content[ticker]["Individual"].analysis.trade_spline[i])
