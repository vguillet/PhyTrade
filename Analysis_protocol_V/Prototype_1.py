...
def prototype_1():
    ...
    big_data.signal_splines_combined = \
        spline.increase_amplitude_signal_spline(big_data.signal_splines_combined, big_data.signal_spline_volume)

    # ========================= TRADING SIGNAL GENERATION ============================

    print(len(big_data.signal_splines_combined))

    buy_dates, sell_dates = spline.calc_spline_trigger(big_data, big_data.signal_splines_combined)

    print("buy_dates:", buy_dates)
    print("len buy_dates:", len(buy_dates))
    print("sell_dates:", sell_dates)
    print("len sell_dates:", len(sell_dates))

    oc.calc_trigger_values(big_data, sell_dates, buy_dates)
    ...
    return big_data.signal_splines_combined, big_data.sell_trigger_dates, big_data.buy_trigger_dates
